...
class GranRunner(object):
  ...
  def __init__(self, config):
    ...
    # ---- load graphs ----
    _allowed_keys = {
      'source_dir', 'filename_glob', 'recursive_glob',
      'sample_from_P', 'prob_threshold',
      'rng_mode', 'rng_fixed_seed', 'limit',
      'input_format', 'is_probability', 'prob_key', 'adj_key'
    }
    ds_kwargs = {k: getattr(self.dataset_conf, k)
                 for k in _allowed_keys if hasattr(self.dataset_conf, k)}

    self.graphs = create_graphs(
        self.dataset_conf.name,
        data_dir=self.dataset_conf.data_path,
        **ds_kwargs
    )
  ...
  def test(self):
    # ensure save_dir points to the weight directory (keep original behavior)
    self.config.save_dir = self.test_conf.test_model_dir
    want_prob = bool(getattr(self.test_conf, 'return_prob', True))
    ...
    if self.config.test.is_test_ER:
        ...
        graphs_gen = [nx.fast_gnp_random_graph(self.max_num_nodes, p_ER, seed=ii)
        ...
        # if only generate, prepare I/O first
        only_gen  = bool(getattr(self.test_conf, 'only_gen', False))
        want_prob = bool(getattr(self.test_conf, 'return_prob', True))

        if only_gen:
            cfg_dir  = getattr(self.test_conf, 'gen_out_dir', 'gen_graphs_npy')
            out_dir  = cfg_dir if os.path.isabs(cfg_dir) else os.path.join(self.config.save_dir, cfg_dir)
            fmt      = (getattr(self.test_conf, 'gen_format', 'npy') or 'npy').lower()  # npy|npz|edgelist
            prefix   = getattr(self.test_conf, 'gen_prefix', 'gen')
            start_i  = int(getattr(self.test_conf, 'gen_start_index', 0) or 0)
            os.makedirs(out_dir, exist_ok=True)
            saved = 0  # number saved

        # batch generation (if only_gen=True, write out during the loop)
        A_pred = []           # for non-only_gen
        num_nodes_pred = []   # for non-only_gen
        ...
        for ii in tqdm(range(num_test_batch)):
            with torch.inference_mode():
                ...
                input_dict = {
                    ...
                    'return_prob': want_prob,
                }

                # fixed number of nodes via one-hot pmf; else use training pmf
                target_n = getattr(self.test_conf, 'target_num_nodes', None)
                if target_n is not None:
                    pmf = torch.zeros(int(self.max_num_nodes), dtype=torch.float32, device=self.device)
                    pmf[target_n - 1] = 1.0
                    input_dict['num_nodes_pmf'] = pmf
                else:
                    input_dict['num_nodes_pmf'] = self.num_nodes_pmf_train

                A_tmp = model(input_dict)   # returns list[Tensor] (each a square matrix)

                gen_run_time += [time.time() - start_time]

                # normalize to list
                if isinstance(A_tmp, torch.Tensor):
                    A_tmp = [A_tmp]
                A_tmp = list(A_tmp)

                if only_gen:
                    for aa in A_tmp:
                        if saved >= self.num_test_gen:
                            break
                        arr = aa.detach().cpu().numpy() if isinstance(aa, torch.Tensor) else np.asarray(aa)

                        idx = start_i + saved
                        path = os.path.join(out_dir, f"{prefix}{idx}.{fmt}")

                        if want_prob:
                            if fmt == 'npy':
                                np.save(path, arr.astype(np.float32))
                            elif fmt == 'npz':
                                np.savez_compressed(path, P=arr.astype(np.float32))
                            else:
                                raise ValueError("When return_prob=True, gen_format must be 'npy' or 'npz'.")
                        else:
                            if fmt in ('npy', 'npz'):
                                A_np = arr.astype(np.uint8)
                                if fmt == 'npy':
                                    np.save(path, A_np)
                                else:
                                    np.savez_compressed(path, A=A_np)
                            elif fmt == 'edgelist':
                                G = nx.from_numpy_array(arr.astype(np.uint8))
                                nx.write_edgelist(G, path, data=False)
                            else:
                                raise ValueError(f"Unsupported gen_format: {fmt}")
                    ...
                    need = self.num_test_gen - len(A_pred)
                    if need <= 0:
                        break
                    A_tmp = A_tmp[:need]
                    A_pred += [aa.detach().cpu().numpy() if isinstance(aa, torch.Tensor) else np.asarray(aa)
                               for aa in A_tmp]
                    num_nodes_pred += [aa.shape[0] for aa in A_tmp]

        logger.info('Average test time per mini-batch = {}'.format(np.mean(gen_run_time)))

    # post-processing for non-only_gen
    if only_gen:
        print(f"[GEN] saved {saved} {'P-matrices' if want_prob else 'graphs'} → {out_dir} (format={fmt}, prefix={prefix})")
        print("only_gen=True: skip visualization and evaluation.")
        return

    if want_prob:
        P_pred = A_pred
        print(f"Returned {len(P_pred)} P-matrices.")
    else:
        graphs_gen = [get_graph(aa) for aa in A_pred]

    # visualization (skipped when only_gen=True)
    if self.is_vis:
        ...
        if self.is_single_plot:
            ...
            draw_graph_list_separate(self.graphs_train[:self.num_vis], fname=save_name[:-4], is_single=True, layout='spring')

    # evaluation
    if want_prob and not getattr(self.test_conf, 'only_gen', False):
      logger.info("return_prob=True & only_gen=False: skip evaluation.")
      return
    ...
    if self.config.dataset.name in ['lobster']:
      return (mmd_degree_dev, mmd_clustering_dev, mmd_4orbits_dev, mmd_spectral_dev,
              mmd_degree_test, mmd_clustering_test, mmd_4orbits_test, mmd_spectral_test, acc)
    else:
      return (mmd_degree_dev, mmd_clustering_dev, mmd_4orbits_dev, mmd_spectral_dev,
