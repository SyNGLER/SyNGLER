...
def main():
    ...
    ap.add_argument("--n", type=int, default=1500, help="Number of nodes.")
    ap.add_argument("--sparse", type=float, default=0.0, help="sparse_level.")
    ap.add_argument("--tau", type=float, default=0.0, help="tau.")
    ap.add_argument("--seed-start", type=int, default=0, help="Start seed.")
    ap.add_argument("--seed-end", type=int, default=0, help="End seed.")
    ap.add_argument(
        "--data-root",
        required=True,
        help=f"Directory containing the input .npy files directly (named 'seed={seed}.npy') and 'lcc.csv'."
    )
    ap.add_argument(
        "--out-root",
        required=True,
        help="Output directory (logs/ckpt/samples/temp YAML will go here). No extra subdirectories are appended."
    )
    ap.add_argument(
        "--just-test",
        action="store_true",
        help="Only generate samples without training (requires test.model in base YAML to point to existing weights)."
    )
    ...
    base_cfg["test"]["target_num_nodes"] = args.n
    ...
    for seed in range(args.seed_start, args.seed_end + 1):
        ...
        # No extra subdirectories under out_root
        out_dir = out_root

        cfg["run_id"] = f"gran_dblp_seed{seed}"
        ...
        tmp_yaml = f"{out_dir}/_auto_gran_dblp_seed_{seed}.yaml"
