...
class GRANMixtureBernoulli(nn.Module):
  ...
  def _sampling(self, B,return_prob = True):
    """ generate adj in row-wise auto-regressive fashion """
    with torch.no_grad():
        ...
        P = torch.zeros(B, N_pad, N_pad).to(self.device)
        ...
        return P if return_prob else A
  ...
  def forward(self, input_dict):
    ...
    """
    return_prob = bool(input_dict.get('return_prob', True))  # NEW
