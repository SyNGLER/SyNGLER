...
def snapshot(model, optimizer, config, step, gpus=[0], tag=None, scheduler=None):
  ...
  snap_path = os.path.join(
      config.save_dir,
      "model_snapshot_{}.pth".format(tag) if tag is not None else
      "model_snapshot_{:07d}.pth".format(step)
  )
  torch.save(model_snapshot, snap_path)

  latest_path = os.path.join(config.save_dir, "model_last.pth")
  try:
    shutil.copyfile(snap_path, latest_path)
  except Exception as e:
    print(f"[snapshot] Warning: failed to write model_last.pth: {e}")
  ...
  config_save.test.test_model_name = "model_last.pth"

  config_save.test.latest_model_dir = config.save_dir
  config_save.test.latest_model_name = "model_last.pth"
