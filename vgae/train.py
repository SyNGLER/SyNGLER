...
from input_simulated import load_simulated_data
...
adj, features = load_simulated_data(args.data_path)
...
net = getattr(model, args.model)(adj_norm)
...
for epoch in range(args.num_epoch):
    ...
    A_pred = net(features)
    optimizer.zero_grad()
    loss = norm * F.binary_cross_entropy(A_pred.view(-1),
                                         adj_label.to_dense().view(-1),
                                         weight=weight_tensor)
    if args.model == 'VGAE':
        kl_divergence = 0.5 / A_pred.size(0) * (1 + 2 * net.logstd - net.mean**2 - torch.exp(net.logstd)**2).sum(1).mean()
        loss -= kl_divergence
...
print("End of training!",
      "test_roc=", "{:.5f}".format(test_roc),
      "test_ap=", "{:.5f}".format(test_ap))

@torch.no_grad()
def generate(model_module, feat, output_dir, num_samples=200, rho_target=None):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    model_module.eval()
    for i in tqdm(range(num_samples), desc="Generating samples"):
        P = model_module(feat).detach()
        if rho_target is not None:
            P = calibrate(P, rho_target)
        else:
            P = (P + P.t()) / 2
            P.fill_diagonal_(0.0)
        np.save(os.path.join(output_dir, f"rep{i}.npy"),
                P.cpu().numpy().astype(np.float32))

generate(net, features, args.output_dir, num_samples=200, rho_target=rho_train)
